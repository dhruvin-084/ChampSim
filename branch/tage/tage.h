...
// 4 Banks plus a basic bimodal predictor
#define NUM_BANKS 4
...
void tage_init(){
    for(int i = NUM_BANKS-1; i >=0; i--){
        GEOMETRICS[NUM_BANKS-i-1] = (int) (MIN_HISTORY_LEN * (powf(float(MAX_HISTORY_LEN) / float(MIN_HISTORY_LEN), (float(i) / float(NUM_BANKS - 1)))) + 0.5);
